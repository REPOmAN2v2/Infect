...
Board ** parseArgs(int argc, char **argv)
{
	...
	for (size_t i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-x") == 0) {
			...
			checkArg(i, argc, argv, &size, map, &countDoc);
		} else if (strcmp(argv[i], "-i") == 0) {
			checkArg(i, argc, argv, &size, map, &countInf);
		} else if (strcmp(argv[i], "-s") == 0) {
			checkArg(i, argc, argv, &size, map, &countSol);
		} else if (strcmp(argv[i], "-n") == 0) {
			checkArg(i, argc, argv, &size, map, &countNur);
...
void printHelp()
{
    ...
    fprintf(stdout, "Default values are x=79, y=20, fast speed, 10 doctors, 3 infected, 1 soldier.\n");
    exit(EXIT_FAILURE);
}
